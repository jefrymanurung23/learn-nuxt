<template>
  <div>
    ...
    <div v-for="tag in tags" :key="tag.id" class="flex-table">
      ...
      <div class="actions">
        <v-btn x-small @click="setToEditing(tag)">
          Edit
        </v-btn>
        <v-btn x-small @click="deleteTag(tag)">
          Delete
        </v-btn>
      </div>
    </div>
    <div v-if="!isEditingNewTag">
      <v-btn @click="startNewTag()">
        Add Tag
      </v-btn>
    </div>
    <div v-else>
...
export default {
  ...
  methods: {
    ...
    deleteTag (tag) {
      const confirmed = confirm(`Are you sure you want to delete tag ${tag.name}? It is connected to ${tag.video_ids.length} videos.`)
      if (confirmed) {
