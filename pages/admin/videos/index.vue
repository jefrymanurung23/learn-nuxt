<template>
  <v-container>
    <h1>Video List</h1>
    <v-btn tile color="primary" to="/admin/videos/new">
      Add Video
    </v-btn>
    ...
    <div v-for="video in videos" :key="video.id" class="flex-table">
      ...
      <div class="actions">
        ...
        <router-link :to="`/admin/videos/${video.id}`">
          Show
        </router-link>
        <router-link :to="`/admin/videos/${video.id}/edit`">
          Edit
        </router-link>
        <v-btn x-small color="error" @click="deleteVideo(video)">
          Delete
        </v-btn>
      </div>
    </div>
